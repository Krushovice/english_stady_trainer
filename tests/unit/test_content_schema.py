...
from app.schemas.content import FinalExamBankFile, LessonFile, PlacementBankFile
from app.services.content_loader import FINAL_EXAM_BANK_DIR_NAME, PLACEMENT_BANK_DIR_NAME

CONTENT_DIR = Path(__file__).resolve().parent.parent.parent / "content"

_BANK_DIR_NAMES = {PLACEMENT_BANK_DIR_NAME, FINAL_EXAM_BANK_DIR_NAME}
...
def _lesson_files() -> list[Path]:
    return [p for p in _all_content_files() if p.parent.name not in _BANK_DIR_NAMES]
...
def _final_exam_bank_files() -> list[Path]:
    return [p for p in _all_content_files() if p.parent.name == FINAL_EXAM_BANK_DIR_NAME]
...
@pytest.mark.parametrize("path", _final_exam_bank_files(), ids=lambda p: p.stem)
def test_final_exam_bank_file_matches_schema(path: Path) -> None:
    raw = yaml.safe_load(path.read_text())
    bank_file = FinalExamBankFile.model_validate(raw)

    assert bank_file.items
    slugs = [item.slug for item in bank_file.items]
    assert len(slugs) == len(set(slugs)), "duplicate slugs in final exam bank"

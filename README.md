...
**A2 is also complete: 10 lessons** (`content/a2/`) — Ordering Food, Asking for Directions, Hotel Check-in, Everyday Problems, Technology, Making Plans, Past Continuous & Storytelling, Superlatives, Gaming & Free Time, Revision. Grammar arc: polite requests (Could I have/I'd like) → imperatives + movement prepositions → Present Perfect (introduction) → have to/must/should → used to → going to vs will → Past Continuous vs Past Simple → superlatives → gerund/infinitive verb patterns → mixed revision. Same structure as A1 (4 main exercises + self-contained `mini_test` per lesson, none on Revision). B2 doesn't exist yet.

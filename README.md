...
Phases 0–4 are done: authentication, DB, test infrastructure, the course tree (levels/modules/lessons/blocks + vocabulary/grammar) with a YAML content loader, a deterministic exercise engine (multiple choice, fill-in-the-blank, sentence ordering, reading comprehension) with attempt history and per-skill progress, a placement test that estimates a CEFR level per skill and recommends starting modules, and learning intelligence — automatic grammar-mistake classification and spaced-repetition review scheduling on every practice attempt. Phase 5 (AI) is done: the provider abstraction, writing feedback, homework generation, conversation mode, and the Speaking flow (prompt → recording → STT → evaluation → feedback → retry) are all built and verified end to end against real local models — see "AI (local model)", "Writing feedback and homework", "Conversation mode", "Speech-to-text (STT)", and "Speaking" below. Content authoring is also done: all 43 planned lessons across A1/A2/B1/B2 are written (see "Content" below); C1/C2 stay explicitly out of scope. Phase 6 (UX) is mostly done: the core learning loop, a real `Dashboard` (do now / weak at / improved / due for review), a grouped sidebar nav (Панель/Уроки, Ежедневный тест/Повторение/Домашнее задание/Тренировка речи/Разговор, Прогресс/Финальный экзамен/Сертификат — see "Frontend" below), progress screen with a computed title/grade, review center, placement test UI, level exit exams, frontend for Homework/Speaking/AI Conversation, a course-wide final exam gating a printable completion certificate, a "C1/C2 — coming soon" placeholder on `/levels`, sequential lesson unlocking with a single-check exercise flow, a placement-driven starting-point choice, real local TTS audio for every lesson/placement listening item, and a full Russian translation of the interface chrome (see "Sequential lessons and the single-check exercise flow", "Placement-driven starting point", "Listening audio (Kokoro-TTS)", and "Russian UI translation" below) are all built (as of 2026-08-22) — this closes out the live-testing-feedback round. Also done (2026-08-22): a full premium visual redesign (editorial typography, a persistent sidebar nav replacing the old top bar, a KrushEnglish brand identity with a real logo), a `users.name` field collected at registration, and a Vitest + React Testing Library frontend test suite (see "Frontend tests" below). Still open: a VPS deployment (see `docs/decisions.md`).
...
- `auth/` — `AuthContext` (JWT in `localStorage`, hydrated via `GET /auth/me` on load). `App.tsx` gates the whole route tree on `user` once instead of a per-route wrapper (the old `ProtectedRoute` was removed 2026-08-22 as redundant once this centralized).
...
- `components/layout/Sidebar.tsx` — persistent left nav grouped Учиться/Практика/Прогресс (Панель, Уроки / Ежедневный тест, Повторение, Домашнее задание, Тренировка речи, Разговор / Прогресс, Финальный экзамен, Сертификат), active-state via `NavLink`, collapses to a horizontal icon strip under 860px. Replaced the old top-bar `Header.tsx` in the 2026-08-22 redesign.
...
```bash
cd frontend
npm test          # vitest run — one-shot, for CI/pre-commit
npm run test:watch  # vitest — watch mode for local development
```

Vitest + React Testing Library (`jsdom` environment), 41 tests across 8 files (2026-08-22) — no MSW or other network-mocking library; `fetch`-backed API modules (`api/exercises.ts`, `api/auth.ts`, ...) are mocked directly with `vi.mock`, which is enough given how thin the API layer already is. Covers: pure helpers (`ExamPage`'s `formatClock`/`formatWhen`), the four exercise input components' answer-reporting and disabled-state behavior, `ExerciseCard`'s full submit/correct/incorrect/retry/network-error flow, `AuthContext`'s session-restore/login/register/logout lifecycle, `RegisterPage`'s form-to-API wiring including the new `name` field, and `DashboardPage`'s "do now" priority branching (due reviews > weak topics > daily quiz > "start a lesson" nudge) plus its name/email-fallback greeting logic. Deliberately not attempted: full-page E2E through a real backend (that's what the ad-hoc Docker-Playwright verification runs throughout this log are for) and exhaustive coverage of every page — this is a foundation to build on, not a completeness claim.
...
- `SpeakingPage` at `/speaking` (nav label "Тренировка речи" — see `docs/decisions.md` for
  why it's not called "Говорение" like the skill-category label elsewhere) — "Get a
  speaking prompt" calls `POST /speaking/prompts`; `AudioRecorder` records the answer,
  then `submitSpeakingAttempt` uploads it (multipart) and shows the STT transcript +
  `WritingFeedbackCard`.
- `ConversationPage` at `/conversation` (nav label "Разговор") — optional topic → chat UI
